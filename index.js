// Arc'teryx Stock Monitor with Discord Webhook Integration
// Specifically designed for Arc'teryx's Next.js structure
...
let previousStock = {};
let checkCount = 0;
let allKnownColors = new Map(); // variantId -> color data
...
function parseStockData(html) {
  ...
  try {
    ...
    stockData.productName = product.analyticsName || 'Bird Head Toque';
...
async function checkVariantStock(variantId) {
  ...
  try {
    const variantUrl = `${CONFIG.PRODUCT_URL}?dwvar_X000006756_color=${variantId}`;
    ...
    const addToCart = $('button:contains("Add to"), button:contains("Shop now")').length > 0;
    
    return !notifyMe && addToCart;
  } catch (error) {
...
async function checkAllVariantStocks() {
  ...
  // Check each variant sequentially (with delays to avoid rate limiting)
...
async function checkStock() {
  ...
  try {
    // Step 1: Fetch main page to get all color variants
    ...
    // Step 2: Check stock status for all variants
